...
class Meet_My_Team_Shortcodes {
	...
	private function build_single_member( $details ){
		...
		if( $details['bio_picture'] != '' ){
			$modal .= 	'<span class="mmt_bio_picture"><img src="'.$details['bio_picture'].'"></span>';
		}

		$modal .=		'<h4 id="mmt_jb_member_name">'.$details['name'].'</h4>
						<h6 id="mmt_jb_member_designation">'.$details['designation'];

		if( $details['dist_descrip'] != '' ){
			$modal .= ' - '.$details['dist_descrip'];
		}
		$modal .= 		'</h6>';
